...
#### Weights (required)
The weights of each grading component must be provided in every configuration file. They are passed as a `HashMap`, with each component (`coverage`, `mutation`, `meta`, and `codechecks`) as the key and its weight as the value. The combined weight of all components must sum up to exactly `1.0`. Unused or ungraded components can have their weight set to 0. For example:
...
The fully-qualified names (FQN) of all classes under test must be provided in the configuration file. This is done as follows:
...
Note: This method does not have to be overridden unless you wish to disable JaCoCo. If it is not overridden, JaCoCo will be enabled.
...
Note: This method does not have to be overridden unless you wish to disable Pitest. If it is not overridden, Pitest will be enabled.

If Pitest is disabled, the weight of the `mutation` grading component must also explicitly be set to 0, as specified above.


*(Advanced)* Unless otherwise specified, Andy uses Pitest's default mutators. Other mutator groups or specific mutators can be specified if desired. Information about mutators and groups can be found on [the Pitest website](https://pitest.org/quickstart/mutators/). Mutator groups can be set as follows:
...
Note: This method usually does not have to be overridden and the default mutator group is sufficient in the vast majority of cases.
...
The following types of meta tests exist:

- String replacement - `MetaTest.withStringReplacement([weight,] name, old, replacement)` - A particular piece of code is searched for and replaced in the class under test. The search is not whitespace-sensitive.
...
--- Code checks
2/10 passed
should have properties: PASS (weight: 2)
either make use of Arbitraries or JQWik IntRange-like annotations: FAIL (weight: 8)
```

#### Required code checks

Required code work in a similar way to regular code checks. However, they are not considered a grading component. Instead, if one or more required code checks fail, the final grade is overridden to 0, irrespective of the score of any of the grading components.
