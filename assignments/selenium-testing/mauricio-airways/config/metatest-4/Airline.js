...
function bookCurrentlySelected() {
    ...
    reservedTickets.push({ flight: flights[getAlternativeID(flightName)], numberOfSeatsBooked: selectedSeats });
    visualizeTickets();
}
...
function getAlternativeID(id) {
    var chosen = availableFlights[Math.floor(Math.random() * availableFlights.length)].id;
    while (chosen == id) {
        chosen = availableFlights[Math.floor(Math.random() * availableFlights.length)].id;
    }
    return chosen;
}
function initialize() {
